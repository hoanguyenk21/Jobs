    ...
    <div class="row">
      ...
      <div class="col-xl-6 col-md-6 col-sm-6">
        <div class="utf-submit-field">
          ...
          <option @if($blog->working_time == $key) selected  @endif value="{{$key}}">{{$val}}</option>
          @endforeach
         
          <option value="Full time,Part time"  @if($blog->working_time == "Full time, Part time") selected  @endif />Full time, Part time</option>
      ...
      <div class="col-xl-6 col-md-6 col-sm-6">
        <div class="utf-submit-field">
          ...
          <input type="number" name="salary_min" value="{{$blog->salary_min}}" class="utf-with-border" placeholder="Mức lương...">
          @error('salary_min')
          <p class="text-danger">{{$message}}</p>
          @enderror
        </div>
      ...
      <div class="col-xl-6 col-md-6 col-sm-6">
        ...
        <div class="utf-submit-field">
          ...
          @foreach(config('common.gender') as $key => $val)
             <option @if($blog->gender == $key) selected  @endif value="{{$key}}">{{$val}}</option>
      ...
      <div class="col-xl-6 col-md-6 col-sm-6">
        <div class="utf-submit-field">
          ...
          @foreach(config('common.exp') as $key => $val)
             <option @if($blog->exp == $key) selected  @endif value="{{$key}}">{{$val}}</option>                        
          @endforeach
          </select>
          @error('exp')
          <p class="text-danger">{{$message}}</p>
      ...
      <div class="col-xl-6 col-md-6 col-sm-6">
        <div class="utf-submit-field">
          ...
          <div class="utf-input-with-icon">
            <input type="date" name="deadline" value="{{$blog->deadline}}" id="deadline" class="form-control" value="">
            @error('deadline')
          <p class="text-danger">{{$message}}</p>
          @enderror
          </div>
      ...
      <div class="col-xl-12 col-md-12 col-sm-12">
        <div class="utf-submit-field">
          ...
          <textarea id="editor" cols="40" rows="2" name="detail" class="utf-with-border" placeholder="Mô tả công việc...">{{$blog->detail}}</textarea>
          @error('detail')
          <p class="text-danger">{{$message}}</p>
          @enderror
        </div>
      ...
      <div class="utf-centered-button">
        <a href="javascript:void(0);" class="button utf-ripple-effect-dark utf-button-sliding-icon margin-top-0"><button type="submit">Lưu</button><i class="icon-feather-plus"></i></a>
