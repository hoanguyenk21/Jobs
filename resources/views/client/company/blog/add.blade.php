  ...
  <form method="POST" enctype="multipart/form-data" class="content with-padding padding-bottom-10">
    ...
    <div class="row">
      ...
      <div class="col-xl-6 col-md-6 col-sm-6">
        <div class="utf-submit-field">
          ...
          @foreach(config('common.working_time') as $key => $val)
              <option 
              @if(old('working_time') == $key) selected  @endif value="{{$key}}">{{$val}}</option>
          @endforeach
            </select>
          @error('working_time')
           <p class="text-danger">{{$message}}</p>
      ...
      <div class="col-xl-6 col-md-6 col-sm-6">
        <div class="utf-submit-field">
          <h5>Kinh nghiệm</h5>
          <select name="exp" class="selectpicker utf-with-border" data-size="7" title="Hình thức làm việc">
          @foreach(config('common.exp') as $key => $val)
          <option @if(old('exp') == $key) 
                        selected 
                      @endif value="{{$key}}">{{$val}}</option>
          @endforeach
            </select>
      ...
      <div class="col-xl-6 col-md-6 col-sm-6">
        <div class="utf-submit-field">
          ...
          <select class="selectpicker utf-with-border" name="location_id" data-size="7" title="Hình thức làm việc">
            @foreach($loca as $l)
            <option @if($l->id == old('location_id'))
                        selected
